...
var api = {
    ...
    getSize: '/selfcheck/serviceqa/getCategoryNum'
}
...
var mainFun = {
    ...
    initList: function(recheck) {
        ...
        HTTP.getProduct(function(res) {
            ...
            //getsize
            HTTP.getSize({title: searchKey}, function (res) {
                var lis = $(".AC_con_pannel ul li");
                var totalsize=0;
                if(res.length) {
                    $.each(res, function (i, o) {   
                       saveNum[o.categoryUuid] = o;
                       totalsize +=saveNum[o.categoryUuid].num;
                    })
                    //问题类型
                    $.each(categoryList, function (i, o) {
                        if(o.uuid == globalUUId) {
                            $("#essize").html(totalsize+'条')
                        }
                        li2+= "<li data-id="+o.uuid+" class="+(o.uuid==globalUUId ? 'active': 0)+"   data-categoryUuid="+o.categoryUuid+" data-parentUuid="+o.parentUuid+">"+o.title+"<em>("+saveNum[o.categoryUuid].num+")</em></li>";
                    });
                    ...
                    if(categoryList.length) {
                        if(recheck) {
                            ...
                            var uuid = globalUUId ? globalUUId :res.categoryList[0].uuid;
                            Tab(uuid);
                            rdom.data('id', uuid)
                        }
                    }
        ...
        function Tab(defaultId) {
            ...
            var page = $("<div id='page' class='page'></div>");
...
function search(value, id) {
    value = $.trim(value)
    if(value) {
        window.location.href = '/pages/serviceRevision/autoCheckSearch.html?key='+value+'&uuid='+(id? id:globalUUId);
    }
}
$init.Ready(function () {
    KUYU.$scope = {};
