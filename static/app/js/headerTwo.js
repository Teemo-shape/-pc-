...
define('KUYU.Header', ['KUYU.Service', 'KUYU.Binder', 'KUYU.Store', 'KUYU.userInfo', 'juicer'], function() {
	...
	function insertAllproduct(element) {
        document.getElementById('j-h-menu').insertAdjacentHTML('beforeEnd', element)
	}
	var headerTwo = {
		getTopBar: function() {
		...
		menuHover: function() {
            ...
            // $init.getHeaders()['ihome-token'] =  $Store.get('istaff_token')
            $http.get({
                url: "/front/product/productcategoryfront/getAllNodesById",
                data: {
                    uuid: 'staff_pc',
                ...
                success: function(data) {
                    ...
                    $.each(data.list,function(i,item){
                        ...
                        if(item.categoryUrl){
                            html+='<li>'+
                                '<a href='+item.categoryUrl+'>'+item.categoryName+'<span class="flex-next">&#xe631;</span></a>';
                        }else{
                            html+='<li>'+
                                '<a href="javascript:;">'+item.categoryName+'<span class="flex-next">&#xe631;</span></a>';
                        }
                        if(item.children&&item.children.length){
                            html+='<div class="child-category">';
                            $.each(item.children,function(j,child){

                                if(child.categoryUrl != undefined){
                                    if(child.categoryName.length > 0){
                                        html+=' <a href='+child.categoryUrl+'><div class="clearfloat">'+
                                            '<div class="level2-category"><span class="level2-category-content">'+child.categoryName+'</span><span class="flex-next">&#xe631;</span></div></a>';
                                    }else{
                                        html+='<div class="clearfloat">'
                                    }
                                }else{
                                    if(child.categoryName.length > 0) {
                                        html += ' <a href="javascript:;"><div class="clearfloat">' +
                                            '<div class="level2-category"><span class="level2-category-content">' + child.categoryName + '</span><span class="flex-next">&#xe631;</span></div></a>';
                                    }else{
                                        html+='<div class="clearfloat">'
                                    }
                                ...
                                if(child.children&&child.children.length){
                                    html+='<ul class="level3-category">';
                                    $.each(child.children,function (k, o) {
                                        html+='<li><a href='+o.categoryUrl+'>'+o.categoryName+'</a></li>'
                                    })
                                    html+='</ul>';
                                }
                                html+='</div>';
                    ...
                    $('#nav_menu').empty().append(html);
                }
            })
            $http.get({
                url: '/homePage/topNavigation/getActiveTopNavigation',
                dataType: 'json',
                // headers:{"ihome-token": localStorage.istaff_token},
                data: topParams,
                success: function (res) {
                    var dom = $("#topads");
                    if(res.retData && res.code == 0) {
                        ...
                        for(var i=0;i<data.length;i++){
                            if(data[i].type==0){
                                ...
                                productsData.push(data[i].topNavigationContents);
															...
															headerHtml+="<div class=\"mes-danlianjie\" data-id="+headermenuindex+"><a href="+data[i].url+">"+data[i].titleName+"</a></div>";
															headermenuindex++;
															productsData.push(data[i].topNavigationContents);
														}
                        }
                        // console.log('document.getElementById(\'j-h-menu\')', document.getElementById('j-h-menu'))
                        insertAllproduct(headerHtml)
                        // setTimeout(function() {
                        //
                        // }, 0)
                        // document.getElementById('j-h-menu').insertAdjacentHTML('beforeEnd', headerHtml)
            ...
            getUserInfo(function(res) {
                ...
                if(res.code == CODEMAP.status.success) {
                    ...
                    var html ='<li class="index_cus_id" ><a href="javascript:;" id="onlyName" data-id=${customerUuid}  >{@if nickName} ${nickName} {@else} {@if realName} ${realName} {@else} ${customerName}{@/if} {@/if}</a><em class="arrow webfont">&#xe608;</em>' +
                        ...
                        '<a href="/pages/usercenterindex/usercenter.html"><span>个人中心</span></a>'+
                        '<a href="/pages/collect/collects.html#collects"><span>我的收藏</span></a>'+
                        '<a href="/pages/usercenterindex/usercenter.html" target="_blank"><span>账号管理</span></a>'+
                    ...
                    headerTwo.getCartCount('login')
                    if(GLOBAL_UUID) { //如果UID存在侧进入的是商品详情客服
                        headerTwo.GETKF(GLOBAL_UUID, res);
                    } else {
                        headerTwo.GETKF(null,res);
                    }
                } else {
...
//	headerTwo.topSearch();
    ...
    _APP.inject('KUYU.HeaderTwo', headerTwo);
});
