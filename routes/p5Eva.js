...
const banners = async () => {
    return $http.get(api.banner, {type: 10} ).then( res => {
        if(res.code === '0') {
...
async function __main__(ctx, next) {
    var token = ctx.cookies.get('_ihome_token_') ? ctx.cookies.get('_ihome_token_') : null;
    var params = {
        'token': token
    }
    ...
    await ctx.render('/components/p5/index', {'__result__':result} )
}
